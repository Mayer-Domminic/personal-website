...
// Define interfaces for our location types
interface Location {
  name: string;
  country: string;
  lat: number;
...
const CustomGlobe = () => {
  ...
  const iter_locations: Location[] = useMemo(() => [
    { name: "Tokyo", country: "Japan", lat: 35.6762, lng: 139.6503, type: "dream", nextUp: true },
    { name: "Kyoto", country: "Japan", lat: 35.0116, lng: 135.7681, type: "dream", nextUp: true },
    { name: "Seoul", country: "South Korea", lat: 37.5665, lng: 126.9780, type: "dream", nextUp: true },
    { name: "Singapore", country: "Singapore", lat: 1.3521, lng: 103.8198, type: "dream" },
        ...
        >
          {dimensions && (
            <Globe
              ...
              pointAltitude={0.08}
              pointColor={d => (d as Location).type === "visited" ? "#60A5FA" : "#c084fc"}
              pointRadius={0.2}
              pointsMerge={true}
              htmlElementsData={locations}
              htmlElement={d => {
                const el = document.createElement('img');
                el.src = customMarker((d as Location).type);
                el.style.width = '24px';
                el.style.height = '24px';
                el.style.translate = '-5% -5%';
                el.title = `${(d as Location).name}, ${(d as Location).country}`;
                return el;
              }}
